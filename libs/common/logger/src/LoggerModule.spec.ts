 ...
 * Mock 中间件消费者类型
 *
 * @description 定义包含所有必需方法的中间件消费者 mock 类型
 */
type MockedMiddlewareConsumer = {
  apply: jest.Mock;
  exclude: jest.Mock;
  forRoutes: jest.Mock;
};
...
describe('LoggerModule', () => {
  ...
  let middlewareConsumer: MockedMiddlewareConsumer;
  ...
  beforeEach(() => {
    ...
    middlewareConsumer = mockChain as MockedMiddlewareConsumer;
  });
  ...
  describe('configure', () => {
    ...
    it('应配置中间件到默认路由', () => {
      loggerModule.configure(
        middlewareConsumer as unknown as MiddlewareConsumer,
      );
    ...
    it('应支持自定义路由配置', async () => {
      ...
      const customModule = LoggerModule.forRoot(
        customParams as unknown as import('./params.js').Params,
      );
    ...
    it('应支持路由排除', async () => {
      ...
      const dynamicModule = LoggerModule.forRoot(
        paramsWithExclude as unknown as import('./params.js').Params,
      );
