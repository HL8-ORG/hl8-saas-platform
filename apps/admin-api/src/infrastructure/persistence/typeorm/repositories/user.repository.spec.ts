...
describe('UserRepository', () => {
  ...
  describe('emailExists', () => {
    it('应该返回 true 当邮箱已存在时', async () => {
      const email = new Email(validEmail);
      const tenantId = new TenantId(validTenantId);

      ormRepository.count.mockResolvedValue(1);

      const result = await repository.emailExists(email, tenantId);
  ...
  describe('save', () => {
    it('应该更新已存在的用户', async () => {
      ...
      const existingEntity = { id: validUserId } as OrmUser;
